        ...
        if (isset($block->context['frontendRendering']) && $block->context['frontendRendering']) {
            ...
            if (!$repeated) {
                add_action('wp_head', function () use ($innerBlocks, $attributes) {
                    echo '<script id="' . $attributes['id'] . '-qt-json" type="application/json">' . json_encode($innerBlocks, JSON_UNESCAPED_SLASHES) . '</script>';
                });
                add_action('wp_head', function () use ($skeleton, $attributes) {
                    echo '<script id="' . $attributes['id'] . '-qt-skeleton" type="application/json">' . json_encode($skeleton, JSON_UNESCAPED_SLASHES) . '</script>';
                });
            }
            // FILTER
        }

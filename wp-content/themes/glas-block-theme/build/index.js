...
new class MyNotes {
  ...
  editNote(e) {
    ...
    if (thisNote.data("state") == "editabe") {
      //make read only
  ...
  deleteNote(e) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      beforeSend: xhr => {
        xhr.setRequestHeader('X-WP-Nonce', gnasData.nonce);
      },
      url: gnasData.root_url + '/wp-json/wp/v2/note/' + thisNote.data('id'),
      type: "DELETE",
      success: response => {
        this.Note.sideUp();
  ...
  createNote(e) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      url: gnasData.root_url + '/wp-json/wp/v2/note/',
      type: "POST",
      data: ourNewPost,
      success: response => {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()(".new-note-body, new-note-title").val('');
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(gnasData.root_url + "/wp-json/gnas/v1/search?term=" + this.searchField.value);
      ...
      this.resultsDiv.innerHTML = `
        <div class="row">
          ...
          <div class="one-third">
            ...
            ${results.programs.length ? '<ul class="link-list min-list">' : `<p>No programs match that search. <a href="${gnasData.root_url}/programs">View all programs</a></p>`}
          ...
          <div class="one-third">
            ...
            ${results.campuses.length ? '<ul class="link-list min-list">' : `<p>Oh dear, no campus matches your search, ah ha! <a href="${gnasData.root_url}/campuses">View all campuses</a></p>`}
              ${results.campuses.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${results.campuses.length ? "</ul>" : ""}

            <h2 class="search-overlay__section-title">Events</h2>
            ${results.events.length ? "" : `<p>Oh dear, no event matches your search, ah ha! <a href="${gnasData.root_url}/events">View all events</a></p>`}

...
if ( ! function_exists( 'mossco_fse_enqueue_scripts' ) ) :
    ...
    wp_enqueue_script( 'mossco_fse-pgia', get_template_directory_uri() . '/pgia/lib/pgia.js', [], '1.0.190', true);

    wp_register_script( 'inline-script-1', '', [], '1.0.190', false );
    ...
    wp_enqueue_script( 'mossco_fse-maps', get_template_directory_uri() . '/assets/js/maps.js', [], '1.0.190', false);

    wp_deregister_script( 'mossco_fse-script' );
    wp_enqueue_script( 'mossco_fse-script', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyDCjQbOAcZR002cGDTl_hchKl_Oo9vI33U&callback=initMap', [], '1.0.190', true);

    wp_deregister_script( 'mossco_fse-header' );
    wp_enqueue_script( 'mossco_fse-header', get_template_directory_uri() . '/assets/js/header.js', [], '1.0.190', true);
    ...
    wp_enqueue_style( 'mossco_fse-tailwind', get_template_directory_uri() . '/tailwind_theme/tailwind.css', [], '1.0.190', 'all');

    wp_deregister_style( 'mossco_fse-custom' );
    wp_enqueue_style( 'mossco_fse-custom', get_template_directory_uri() . '/custom.css', [], '1.0.190', 'all');

    wp_deregister_style( 'mossco_fse-fancybox' );
    wp_enqueue_style( 'mossco_fse-fancybox', get_template_directory_uri() . '/assets/css/fancybox.css', [], '1.0.190', 'all');

    wp_deregister_style( 'mossco_fse-bgimages' );
    wp_enqueue_style( 'mossco_fse-bgimages', get_template_directory_uri() . '/assets/css/bg-images.css', [], '1.0.190', 'all');

    wp_deregister_style( 'mossco_fse-style' );
    wp_enqueue_style( 'mossco_fse-style', get_bloginfo('stylesheet_url'), [], '1.0.190', 'all');
...
function mossco_fse_blocks_init() {
    ...
    require_once 'blocks/mo-nav-links/mo-nav-links_register.php';
    ...
    require_once 'blocks/mo-cafe-menu-left/mo-cafe-menu-left_register.php';
    require_once 'blocks/mo-cafe-menu-right/mo-cafe-menu-right_register.php';
    require_once 'blocks/mo-menu-nav-links/mo-menu-nav-links_register.php';
    require_once 'blocks/mo-menu-header/mo-menu-header_register.php';
...
$categories = array_merge( $categories, array( array(
        'slug' => 'mo_blocks',
        'title' => __( 'Mossco Blocks', 'mossco_fse' )
    ) ) );

$categories = array_merge( $categories, array( array(
        'slug' => 'mo_menu_blocks',
        'title' => __( 'mo_menu_blocks', 'mossco_fse' )
    ) ) );
